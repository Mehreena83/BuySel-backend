...
class MyPropertyListCreateView(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        total_image_count = (
            (1 if main_image else 0) + len(gallery_images)
        )
...
class MyPropertyDetailView(
    ...
    def update(self, request, *args, **kwargs):
        ...
        remaining_gallery_count = (
            property_obj.images.count()
            - len(gallery_images_to_remove)
        )
        ...
        if not final_main_exists:
            return Response(
                {
                    "main_image": [
                        "A main property image is required."
                    ]
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        old_main_name = (
            property_obj.main_image.name
            if property_obj.main_image
            else None
        )
        old_main_storage = (
            property_obj.main_image.storage
            if property_obj.main_image
            else None
        )
